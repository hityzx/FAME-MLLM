...
def _compat_qwen3_vl_mixed_data(model, processor, is_moe: bool = False):
    ...
    def _inject_fava_embeddings(self, input_ids, inputs_embeds, fava_pixel_values_videos):
        if not getattr(self.config, 'use_fava', False):
            return inputs_embeds

        if fava_pixel_values_videos is None:
            return inputs_embeds

        if input_ids is None:
            raise ValueError('input_ids must be provided when using FaVA placeholder tokens.')

        if getattr(self.config, 'fava_token_id', None) is None:
            raise ValueError('config.fava_token_id must be set when use_fava=True.')

        if not hasattr(self, 'fava') or self.fava is None:
            raise ValueError('FaVA is enabled in config, but self.fava is missing.')

        if not hasattr(self, 'fava_projector') or self.fava_projector is None:
            raise ValueError('FaVA is enabled in config, but self.fava_projector is missing.')
        ...
        if num_placeholders == 0:
            raise ValueError(
                'fava_pixel_values_videos is provided, but no FaVA placeholder token is found in input_ids.'
            )

        # FaVA should run in fp32, even if the Qwen3-VL backbone runs in bf16/fp16.
        ...
        if num_placeholders != fava_embeds.shape[0]:
            raise ValueError(
                ...
                f'FaVA placeholders, but FaVA produced {fava_embeds.shape[0]} embeddings. '
    ...
    def _compute_semantic_decorr_loss_from_inputs(self, input_ids, inputs_embeds):
        ...
        for b in range(batch_size):
            ...
            if (
                max_video_tokens is not None
                and max_video_tokens > 0
                and video_embeds.shape[0] > max_video_tokens
            ):
                idx = torch.linspace(
                    0,
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        inputs_embeds = _inject_fava_embeddings(
            self,
            input_ids=input_ids,
            inputs_embeds=inputs_embeds,
            fava_pixel_values_videos=fava_pixel_values_videos,
        ...
        semantic_decorr_loss = _compute_semantic_decorr_loss_from_inputs(
            self,
            input_ids=input_ids,
            inputs_embeds=inputs_embeds,
        )
